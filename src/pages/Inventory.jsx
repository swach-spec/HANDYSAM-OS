...
export default function Inventory() {
  ...
  async function stockIn(p) {
    ...
    await supabase.from('handysam_stock_moves').insert({ date, product_id: p.id, sku: p.sku, description: p.description, type: 'in', qty, note });
    loadAll();
  }
  ...
  async function stockAdjustment(p) {
    ...
    await supabase.from('handysam_stock_moves').insert({ date, product_id: p.id, sku: p.sku, description: p.description, type: 'adjustment', qty: delta, note: reason });
    loadAll();
  }
  ...
  async function stockRetake(p) {
    ...
    await supabase.from('handysam_stock_moves').insert({
      date, product_id: p.id, sku: p.sku, description: p.description, type: 'retake', qty: variance,
  ...
  return (
    <>
      <div className="card">
        ...
        <table>
          <thead><tr><th>SKU</th><th>Product</th><th>Category</th><th>UOM</th><th>Stock</th><th>Status</th></tr></thead>
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
                <td className="muted">{p.sku}</td><td>{p.description}</td><td className="muted">{p.category}</td><td>{p.uom}</td>
                <td>{p.stock || 0}</td>
      ...
      <div className="card">
        ...
        <table>
          ...
          <tbody>
            {moves.map(m => (
              <tr key={m.id}>
                <td>{m.date}</td><td className="muted">{m.sku || '—'}</td><td>{m.description}</td>
