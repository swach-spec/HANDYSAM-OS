-- New role: sales — POS only, nothing else.
alter table handysam_user_roles drop constraint if exists handysam_user_roles_role_check;
alter table handysam_user_roles add constraint handysam_user_roles_role_check
  check (role in ('admin','accountant','procurement','sales'));
...
begin
  if handysam_current_role() not in ('admin','accountant','sales') then
